...
[ -f decouple-config.sh ] && . decouple-config.sh

SAVEDIR=${SAVEDIR:-/root/.decouple-saved}

if [ -f ${SAVEDIR}/recoupled ]; then
    rd=$(stat -c %Y ${SAVEDIR}/recoupled)
    dd=$(stat -c %Y ${SAVEDIR}/servers.conf)
    nd=$(date +%s)
    rdiff=$(expr $(expr $nd - $rd) / 86400)
    dver=$(head -1 ${SAVEDIR}/decouple-version)
    recommend=2
    [ $rdiff -gt 30 ] && recommend=1
    [ $dver -lt $_VERSION ] && recommend=1
    cat <<RECOUPLECHECK

The decouple save directory appears to have already been used to recouple. If
it has been some time since your last decouple/recouple, or you decoupled with
a much earlier version of this project, the saved configuration could be stale
and invalid.

    1) CLEAR the previously-saved data and start fresh (recommended if it has
       been a long time since you recoupled (e.g. more than a few months).
    2) KEEP the previously-saved data (recommended if you just recently re-
       coupled and are now re-decoupling).
    3) Exit now; no changes will be made.

Decouple data saved on $( date -d @${dd} ) using version $(head -1 ${SAVEDIR}/decouple-version)
     Recoupled last on $( date -d @${rd} ) $rdiff days ago
Recommended answer: ${recommend}. If you're not sure, "2" (KEEP) is the safest option.
RECOUPLECHECK
    while true; do
    ...
    [ "$ans" == "3" ] && { echo "Exiting without changes."; exit 255; }
    [ "$ans" == "1" ] && rm -f ${SAVEDIR}/*
    # recoupled file is removed later (when decouple succeeds)
    echo ""
fi

askyn ans "Ready to go. Decouple this unit from Vera/eZLO cloud services [Y/N]? "
[ "${ans:-}" == "Y" ] || { echo "Exiting without changes."; exit 255; }
